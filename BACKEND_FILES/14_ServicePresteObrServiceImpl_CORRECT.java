...
import com.globalportservices.erp_be.settings.entity.Service;
...
import com.globalportservices.erp_be.settings.repository.ServiceRepository;
...
/**
 * ServicePreste OBR Integration Service - CORRECT VERSION
 *
 * Based on actual database structure where:
 * - Multiple ServicePreste rows have the same NumFacture
 * - Each row represents one service article
 * - ServiceId links to Service master table for service name
 */
@SpringService
...
public class ServicePresteObrServiceImpl {
    ...
    private final ServiceRepository serviceRepository;  // For getting service names
     ...
     * @param lettreTransp Transport letter (e.g., "GPSB17453/25")
     * @return First ServicePreste row with OBR data updated
     */
    @Transactional
    public ServicePreste validateWithOBR(String numFacture, String lettreTransp) {
        ...
        // 1. Find ALL ServicePreste rows for this invoice
        List<ServicePreste> allServiceItems = servicePresteRepository.findAllByNumFacture(numFacture);

        if (allServiceItems.isEmpty()) {
            throw new RuntimeException("No ServicePreste records found for invoice " + numFacture);
        }
        ...
        ServicePreste mainServicePreste = allServiceItems.get(0);

        // 2. Set validation date on ALL items
        LocalDateTime now = LocalDateTime.now();
        for (ServicePreste sp : allServiceItems) {
            sp.setDateValidation(now);
            sp.setIsValid(true);
        }

        log.info("Set validation date for {} ServicePreste items", allServiceItems.size());
        ...
        // 4. Save ALL ServicePreste items
        servicePresteRepository.saveAll(allServiceItems);

        log.info("ServicePreste {} saved with {} items - statusEnvoiOBR: {}",
            numFacture,
     ...
     * Maps each ServicePreste row as a separate invoice_item
     */
    private AddInvoiceRequest buildObrInvoiceRequest(ServicePreste mainItem, List<ServicePreste> allItems) {
        ...
        // Build invoice items from ALL ServicePreste rows
    ...
    /**
     * BUILD INVOICE ITEMS
     * Each ServicePreste row becomes one invoice_item
     */
    private List<InvoiceItemDto> buildInvoiceItems(List<ServicePreste> serviceItems) {
        List<InvoiceItemDto> items = new ArrayList<>();

        log.debug("Building {} invoice items", serviceItems.size());

        for (ServicePreste sp : serviceItems) {
            if (sp.getMontant() == null || sp.getMontant() <= 0) {
                log.warn("Skipping ServicePreste {} with zero/null montant", sp.getServicePresteId());
                continue;
            }

            // Get service name from Service master table
            String serviceName = getServiceName(sp.getServiceId());

            // Calculate amounts
            Double montantTotal = sp.getMontant();  // Total amount (HTVA)
            Double montTaxe = sp.getMontTaxe() != null ? sp.getMontTaxe() : 0.0;
            Double montRedev = sp.getMontRedev() != null ? sp.getMontRedev() : 0.0;
            Double totalTVA = montTaxe + montRedev;
            Double montantTTC = montantTotal + totalTVA;  // Total with taxes

            InvoiceItemDto item = new InvoiceItemDto();
            item.setItem_designation(serviceName);
            item.setItem_quantity("1");
            item.setItem_price(String.format("%.0f", Math.round(montantTotal)));
            item.setItem_ct("0");
            item.setItem_tl("0");
            item.setItem_price_nvat(String.format("%.0f", Math.round(montantTotal)));
            ...
            log.debug("Added item: {} - Montant: {}, TVA: {}, Total: {}",
                serviceName, montantTotal, totalTVA, montantTTC);
        }
    ...
    private String getServiceName(Long serviceId) {
        ...
        return serviceRepository.findById(serviceId)
            .map(Service::getNom)  // Or whatever field has the service name
            .orElse("Service ID: " + serviceId);
    }
