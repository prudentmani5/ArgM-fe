...
const RapportCompteReport: React.FC = () => {
    ...
    useEffect(() => {
        if (dataComptes) {
            setComptes(Array.isArray(dataComptes) ? dataComptes : [dataComptes]);
        }
    }, [dataComptes]);
    ...
    const handleGenerate = async () => {
        ...
        if (!compteCode) {
            toast.current?.show({ severity: 'warn', summary: 'Attention', detail: 'Veuillez sélectionner un compte', life: 3000 });
            return;
        }
        ...
        try {
            ...
            a.href = url;
            a.download = 'rapport_compte.pdf';
            a.click();
            window.URL.revokeObjectURL(url);

            toast.current?.show({ severity: 'success', summary: 'Succès', detail: 'Rapport Compte généré avec succès', life: 3000 });
        } catch (err: any) {
    ...
    return (
        <div className="card p-3 mt-2">
            ...
            <div className="card p-fluid">
                <h5><i className="pi pi-search mr-2"></i>Consultation Compte</h5>
                <div className="formgrid grid">
                    <div className="field col-12 md:col-4">
                        <label htmlFor="compteCode">Compte</label>
                        <Dropdown id="compteCode" value={compteCode} options={comptes} optionLabel="codeCompte"
                            optionValue="codeCompte" onChange={(e) => setCompteCode(e.value)}
                            placeholder="Sélectionner un compte" filter showClear className="w-full"
