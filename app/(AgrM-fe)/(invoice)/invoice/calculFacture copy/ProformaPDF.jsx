// components/ProformaPDF.jsx
import { Page, Text, View, Document, StyleSheet, PDFViewer, Image } from '@react-pdf/renderer';
import { InvoicePDFProps } from '../calculFacture/pdfTypes';
...
const formatCurrencyDevise = (value) => {
    ...
    return new Intl.NumberFormat('fr-FR', {
        style: 'currency',
        currency: 'USD',
        currencyDisplay: 'narrowSymbol', // Pour avoir juste le symbole $
        useGrouping: true,
        minimumFractionDigits: 0,
        maximumFractionDigits: 0
    })
...
export const ProformaPDF = ({ invoice = {} }) => {
    ...
    const getInvoiceTitle = () => {
        if (isFactureValidee) {
            return `FACTURE VALIDÉE N° ${invoice.sortieId || invoice.numFacture || 'N/A'}`;
        }
        return `FACTURE PROFORMA ${invoice.typeFacture || ''} N° ${invoice.sortieId || invoice.numFacture || 'N/A'}`;
    };
    ...
    const renderCalculationDetails = (item) => {
        switch (item) {
            ...
            case 'montMagasinage':
                const prixUnitaire = invoice.montMag37 / poidsT || invoice.montMag / poidsT || 0;
                ...
                if (isGardiennage) {
                    ...
                    if ((invoice.montGardienage37 > 0 || invoice.montGardienage37 === 0) && invoice.duree37 > 0) {
                        if (details) details += ' + ';
                        details += `${formatNumber(invoice.nbreColis || 0)} × ${formatCurrency(invoice.montMag37)} × ${invoice.duree37 || 0}j`;
                    }
                    return details || 'Gardiennage - Calcul basé sur nombre de colis';
                } else {
                    // MAGASINAGE (Autres marchandises)
                    let details = '';
                    ...
                    if ((invoice.montMagasinage37 > 0 || invoice.montMagasinage37 === 0) && invoice.duree37 > 0) {
                        if (details) details += ' + ';
                        details += `${formatCurrencyNumber (tonnageArrondi)} × ${formatCurrency(invoice.montMag7)} × ${invoice.duree37 || 0}j`;
                    }
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <Text style={styles.invoiceTitle}>
                    FACTURE PROFORMA {invoice.typeFacture || ''} N° {invoice.sortieId || invoice.numFacture || 'N/A'}
                </Text>
                    ...
                    <View style={[styles.section, styles.column]}>
                        ...
                        {/* <View style={styles.row}>
                            ...
                            <Text style={styles.value}>
                                {invoice.typeConditionId === 'VE' ? 'Véhicule' : 'Marchandise standard'}
                            </Text>
                        </View> */}
                        <View style={styles.row}>
                ...
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>PARAMÈTRES DE CALCUL COMPLETS</Text>
                    <View style={styles.parameterGrid}>
                        <View style={styles.parameterItem}>
                            <Text style={styles.boldText}>Poids Total: {formatNumber(poidsT)} T</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text style={styles.boldText}>Poids en Kg: {formatNumber(poidsKg)} kg</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text style={styles.boldText}>Tonnage Arrondi: {formatNumber(tonnageArrondi)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Nombre Colis: {formatNumber(invoice.nbreColis || 0)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Tarif Barge: {formatCurrency(tarifBarge)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Tarif Camion: {formatCurrency(tarifCamion)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Surtaxe CLT: {formatCurrency(surtaxeClt)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Frais SRSP: {formatCurrency(fraisSrsp)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Frais Arrimage: {formatCurrency(fraisArrimage)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Nombre Palettes: {formatNumber(nbrePalette)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Nombre Étiquettes: {formatNumber(nbreEtiquette)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Durée normale: {invoice.duree || 0} jours</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Durée étendue: {invoice.duree37 || 0} jours</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Prix unitaire: {formatCurrency(invoice.montMag || 0)}</Text>
                        </View>
                        <View style={styles.parameterItem}>
                            <Text>Prix unitaire 37: {formatCurrency(invoice.montMag37 || 0)}</Text>
                        </View>
                    </View>
                </View>
                */}
                ...
                <View style={styles.signature}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 7 }}>Facturé par: {invoice.userCreation || 'Système'}</Text>
