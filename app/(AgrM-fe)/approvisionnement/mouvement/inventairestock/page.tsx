...
export default function StkInventairePage() {
    const [inventaire, setInventaire] = useState<StkInventaire>(() => {
        ...
        newInventaire.dateInventaire = new Date();
    ...
    // Fonction pour formater les dates en ISO string (compatible avec LocalDateTime)
    const formatDateForBackend = (date: Date | null | undefined): string | null => {
        if (!date) return null;
        return date.toISOString(); // Format: "2025-10-21T15:19:17.730Z"
    };
        ...
        try {
            ...
            if (existingInventaireId) {
                ...
                await axios.put(`${BASE_URL}/update/${existingInventaireId}`, {
                    ...inventaire,
                    dateInventaire: formatDateForBackend(inventaire.dateInventaire),
                    dateUpdate: formatDateForBackend(now),
                    userUpdate: "system"
                });
                ...
                const detailsToSave = details.map(detail => ({
                    ...
                    datePeremption: formatDateForBackend(detail.datePeremption)
                }));
                ...
                const inventaireResponse = await axios.post(`${BASE_URL}/new`, {
                    ...
                    inventaireId: null,
                    dateInventaire: formatDateForBackend(inventaire.dateInventaire),
                    dateCreation: formatDateForBackend(now),
                    dateUpdate: formatDateForBackend(now),
                    userCreation: "system",
                    userUpdate: "system"
                });
    ...
    const handleSubmitEdit = async () => {
        try {
            ...
            await axios.put(`${BASE_URL}/update/${inventaireEdit.inventaireId}`, {
                ...inventaireEdit,
                dateInventaire: formatDateForBackend(inventaireEdit.dateInventaire),
                dateUpdate: formatDateForBackend(now),
                userUpdate: "system"
            });
