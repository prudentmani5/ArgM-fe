...
export default function StkEntreePage() {
    ...
    // Fonction modifiée pour supprimer de la base de données
    const removeDetail = async (index: number, entreeDetailsId?: number, articleName?: string) => {
        const articleLabel = articleName || "cet article";
        
        if (entreeDetailsId) {
            try {
                // Supprimer de la base de données
                await axios.delete(`${BASE_URLD}/delete/${entreeDetailsId}`);
                accept('success', 'Succès', `${articleLabel} a été supprimé avec succès`);
            } catch (error) {
                console.error("Erreur lors de la suppression de l'article:", error);
                accept('error', 'Erreur', `Échec de la suppression de ${articleLabel}`);
                return; // Ne pas supprimer localement si l'API échoue
            }
        } else {
            // Pour les articles qui n'ont pas encore été sauvegardés
            accept('success', 'Succès', `${articleLabel} a été supprimé`);
        }
    ...
    // Fonction modifiée pour supprimer de la base de données dans l'édition
    const removeDetailEdit = async (index: number, entreeDetailsId?: number, articleName?: string) => {
        const articleLabel = articleName || "cet article";
                ...
                <div className="grid">
                    <div className="col-12">
                        ...
                        <StkEntreeForm
                            ...
                            removeDetail={removeDetailEdit} // Fonction modifiée
            ...
            >
                <TabPanel header="Nouvelle entrée" className="p-tabview-panel">
                    ...
                    <StkEntreeForm
                        ...
                        addDetail={addDetail}
                        removeDetail={removeDetail} // Fonction modifiée
