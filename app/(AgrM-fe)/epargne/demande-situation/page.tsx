...
import { StatementRequest, StatementRequestClass, StatementRequestStatus } from './StatementRequest';
import StatementRequestForm from './StatementRequestForm';
import PrintableStatementReceipt from './PrintableStatementReceipt';
...
function SituationRequestPage() {
    ...
    useEffect(() => {
        if (comptesApi.data) {
            ...
            // Auto-select 707 for SITUATION
            const autoSelect707 = mapped.find((c: any) => c.codeCompte?.startsWith('707'));
            if (autoSelect707) {
                setRequest(prev => prev.feeAccountId ? prev : { ...prev, feeAccountId: autoSelect707.compteId });
            }
        }
    ...
    useEffect(() => {
        if (actionsApi.data) {
            switch (actionsApi.callType) {
                case 'create':
                    showToast('success', 'Succès', 'Demande de situation créée avec succès');
                    resetForm();
                    loadRequests();
                    setActiveIndex(1);
                    break;
                ...
                case 'cancel':
                    ...
                    break;
                case 'deliver':
                    showToast('success', 'Succès', 'Situation livrée au client');
    ...
    const validateForm = (): boolean => {
        ...
        if (!request.feeAccountId) { showToast('warn', 'Attention', 'Veuillez sélectionner le compte comptable de revenus (ex: 707)'); return false; }
    ...
    const handleSubmit = () => {
        ...
        const requestData = {
            ...
            feeAccountCode: selectedCompte?.codeCompte || '707',
    ...
    const resetForm = () => {
        const autoSelect = comptesComptables.find((c: any) => c.codeCompte?.startsWith('707'));
    ...
    const openPrintDialog = (rowData: StatementRequest) => { setSelectedRequest(rowData); setPrintDialog(true); };
    ...
    const header = (
        <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
            <h5 className="m-0">Demandes de Situation</h5>
    ...
    return (
        <div className="card">
            ...
            <h4 className="text-primary mb-4">
                ...
                Demande de Situation de Compte
            </h4>
                ...
                <div className="grid">
                    ...
                    <div className="col-12 md:col-4">
                        <div className="flex align-items-center gap-2">
                            ...
                            <span><strong>Compte revenus:</strong> 707 (Frais de demande situation)</span>
                        </div>
                    </div>
                    <div className="col-12 md:col-4">
                        <div className="flex align-items-center gap-2">
            ...
            <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
                ...
                <TabPanel header="Liste des Demandes" leftIcon="pi pi-list mr-2">
                    <DataTable value={requests} paginator rows={10} rowsPerPageOptions={[5, 10, 25, 50]} loading={loading} globalFilter={globalFilter} header={header} emptyMessage="Aucune demande de situation trouvée" stripedRows showGridlines size="small" sortField="requestDate" sortOrder={-1}>
                ...
                {can('EPARGNE_SITUATION_VIEW_TODAY') && <TabPanel header="Demandes du Jour" leftIcon="pi pi-calendar mr-2">
                    <DataTable
                        ...
                        header={
                            <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
                                <h5 className="m-0">Demandes de Situation du Jour</h5>
                                <span className="p-input-icon-left">
                                    <i className="pi pi-search" />
                                    <InputText value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Rechercher..." />
                                </span>
                        ...
                        emptyMessage="Aucune demande de situation pour aujourd'hui"
                ...
                {can('EPARGNE_SITUATION_VIEW_PERIOD') && <TabPanel header="Mes Demandes par Période" leftIcon="pi pi-filter mr-2">
                    {(() => {
                            ...
                            <DataTable
                                ...
                                header={
                                    <div className="flex flex-column gap-2">
                                        <div>
                                            <h5 className="m-0">Mes Demandes de Situation par Période</h5>
            ...
            {/* Deliver Dialog */}
            <Dialog header="Livraison de la Situation" visible={deliverDialog} style={{ width: '450px' }} onHide={() => setDeliverDialog(false)}
            ...
            {/* Print Dialog */}
            <Dialog header="Aperçu du Reçu" visible={printDialog} style={{ width: '900px' }} onHide={() => setPrintDialog(false)}
