...
function GestionCaissePage() {
    ...
    const handleAcknowledgeReceipt = (virementId: number) => {
        confirmDialog({
            message: 'Confirmez-vous la reception des fonds?',
            header: 'Accuser Reception',
            icon: 'pi pi-check-circle',
            acceptLabel: 'Oui, confirmer',
            rejectLabel: 'Annuler',
            accept: () => {
                fetchAck({ userAction: getUserAction() }, 'POST', BASE_URL + '/acknowledge-receipt/' + virementId, 'acknowledge');
            }
        });
    };
    ...
    const handleTransfer = () => {
        ...
        confirmDialog({
            ...
            accept: () => {
                ...
                fetchTransfer({ userAction: getUserAction() }, 'POST', BASE_URL + '/transfer?' + params.toString(), 'transfer');
            }
        });
    };

    const isManager = (myCaisse && (myCaisse.typeCaisse === 'CHEF_AGENCE' || myCaisse.typeCaisse === 'AGENCE' || myCaisse.typeCaisse === 'SIEGE'))
    ...
    return (
        <div className="card">
            ...
            <h2><i className="pi pi-wallet mr-2" />Gestion de Caisse - Guichet</h2>
                ...
                <div>
                    <div className="mb-3 p-3 border-round bg-blue-50 border-blue-200" style={{ border: '1px solid' }}>
                        ...
                        <span className="text-blue-800">
                            Vue branche - Aucune caisse principale (CHEF_AGENCE) detectee. Vous pouvez valider les fermetures des guichets ci-dessous.
                        </span>
                    </div>
                ...
                <>
                    ...
                    <TabView activeIndex={activeTab} onTabChange={(e) => setActiveTab(e.index)}>
                        ...
                        <TabPanel header="Ma Caisse" leftIcon="pi pi-home mr-2">
                            <div className="grid">
                                ...
                                <div className="col-12 md:col-7">
                                    <h4>Comptage des especes</h4>
                                    <DataTable value={DENOMINATIONS} size="small" showGridlines>
                                        <Column field="label" header="Denomination" style={{ width: '40%' }} />
                                        <Column
                                            header="Quantite"
                                            style={{ width: '30%' }}
                                            body={(d: any) => (
                                                <InputNumber
                                                    ...
                                                    onValueChange={(e) => setCashCount(prev => ({ ...prev, [d.field]: e.value || 0 }))}
                                                    min={0}
                                                    size={5}
                                                    className="w-full"
                                                />
                                        ...
                                        <Column
                                            ...
                                            body={(d: any) => formatNumber(((cashCount as any)[d.field] || 0) * d.value)}
                                        />
                                    </DataTable>
                                </div>
                                ...
                                <div className="col-12 md:col-5">
                                    <div className="p-3 surface-100 border-round mb-3">
                                        <h4 className="mt-0">Resume du comptage</h4>
                                        <div className="flex justify-content-between mb-2">
                                            <span>Total physique:</span>
                                            <span className="font-bold text-xl">{formatNumber(calculatePhysicalTotal())} FBu</span>
                                        </div>
                                        <div className="flex justify-content-between mb-2">
                                            <span>Solde theorique:</span>
                                            <span className="font-bold">{formatNumber(myCaisse.soldeActuel)} FBu</span>
                                        </div>
                                        <hr />
                                        <div className="flex justify-content-between">
                                            <span>Ecart:</span>
                                            <span className={`font-bold ${Math.abs(calculatePhysicalTotal() - (myCaisse.soldeActuel || 0)) > 0.01 ? 'text-red-500' : 'text-green-600'}`}>
                                                {formatNumber(calculatePhysicalTotal() - (myCaisse.soldeActuel || 0))} FBu
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    {/* Pending receipts section (caissier acknowledges incoming transfers) */}
                                        ...
                                        <div className="mt-3 p-3 border-round" style={{ border: '1px solid var(--orange-200)', background: 'var(--orange-50)' }}>
                                            <div className="flex align-items-center justify-content-between mb-3">
                                                <div className="flex align-items-center gap-2">
                                                    ...
                                                    <h5 className="m-0 text-orange-700">Fonds en attente de reception</h5>
                                                </div>
                                            ...
                                            <DataTable value={pendingReceipts} size="small" showGridlines>
                                                ...
                                                <Column header="Action" body={(row: VirementInterne) => (
                                                    <Button
                                                        ...
                                                        severity="success"
                                                        size="small"
                                                        onClick={() => handleAcknowledgeReceipt(row.virementId)}
                                                        loading={ackLoading}
                                                    />
                                                )} />
                                            </DataTable>
                            ...
                            <TabPanel header="Ouverture / Fermeture" leftIcon="pi pi-lock-open mr-2">
                                <div className="grid">
                                    ...
                                    {ofSelectedCaisse && (() => {
                                        ...
                                        const theorique = summary?.effectiveBalance ?? ofSelectedCaisse.soldeActuel ?? 0;
                                        ...
                                        <>
                                            ...
                                            <div className="col-12 md:col-8">
                                                ...
                                                <p className="text-sm text-500 mb-3">
                                                    ...
                                                    {isOpening
                                                        ? 'Comptez les especes physiquement. Le total doit etre egal au montant theorique (ecart = 0).'
                                                        : 'Comptez les especes restantes en fin de journee. Un ecart sera signale.'}
                                                </p>
                                                <div className="grid">
                                                    {DENOMINATIONS.map(d => (
                                                        <div key={d.field} className="col-12 md:col-6 lg:col-4">
                                                            <div className="field">
                                                                ...
                                                                <div className="p-inputgroup">
                                                                    <InputNumber
                                                                        value={(ofCashCount as any)[d.field] || 0}
                                                                        onValueChange={(e) => setOfCashCount(prev => ({ ...prev, [d.field]: e.value || 0 }))}
                                                                        min={0}
                                                                        className="w-full"
                                                                    />
                                                                    <span className="p-inputgroup-addon text-sm" style={{ minWidth: '90px' }}>
                                                                        = {formatNumber(((ofCashCount as any)[d.field] || 0) * d.value)}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="col-12 md:col-4">
                                                ...
                                                <div className="surface-card shadow-1 p-3 border-round mb-3">
                                                    ...
                                                    <div className="mt-2">
                                                        <div className="text-500 text-sm mb-1">
                                                            ...
                                                            {isOpening && <span className="text-xs ml-1" style={{ color: '#1565C0' }}>— doit etre 0</span>}
                                                        </div>
                                                    ...
                                                    {isOpening && hasEnteredCount && ecartSignificant && (
                                                        <div className="mt-2 p-2 border-round text-xs" style={{ background: '#FFEBEE', color: '#C62828', borderLeft: '3px solid #C62828' }}>
                                                            ...
                                                            <strong>Ecart non autorise a l'ouverture.</strong> Le comptage physique ({formatNumber(ofTotal)} FBu)
                                                            doit correspondre exactement au montant theorique ({formatNumber(theorique)} FBu).
                                                        </div>
                                                    )}
                                                    ...
                                                    {isOpening && ecartOk && (hasEnteredCount || theorique === 0) && (
                                                        <div className="mt-2 p-2 border-round text-xs" style={{ background: '#E8F5E9', color: '#1B5E20', borderLeft: '3px solid #2E7D32' }}>
                                                            ...
                                                            <strong>Parfait ! Ecart = 0.</strong> Vous pouvez ouvrir la caisse.
                                                        </div>
                                                    )}
                                                    {/* Closing: écart warning */}
                                                ...
                                                <div className="field">
                                                    ...
                                                    <InputTextarea
                                                        value={ofCashCount.notes || ''}
                                                        onChange={(e) => setOfCashCount(prev => ({ ...prev, notes: e.target.value }))}
                                                        rows={3}
                                                        className="w-full"
                                                        placeholder={ecartSignificant && !isOpening ? 'Expliquez l\'ecart constate (obligatoire)...' : 'Remarques eventuelles...'}
                                                    />
                                                </div>
                                                ...
                                                <div className="flex gap-2">
                                                    {isOpening && (
                                                        <Button
                                                            ...
                                                            tooltip={
                                                                ...
                                                                : ecartSignificant ? `Ecart de ${formatNumber(ecart)} FBu — le comptage doit correspondre`
                                                                : ''
                                                            }
                                                    ...
                                                    {!isOpening && (
                                                        <Button
                                                            ...
                                                            disabled={!canClose}
                                                            tooltip={!canClose ? 'Saisissez le comptage physique avant de fermer' : ''}
                                                            tooltipOptions={{ position: 'top' }}
                                                        />
                                                    )}
                                                </div>
                                    ...
                                    <div className="col-12 md:col-5">
                                        <Card title="Virement Interne">
                                            ...
                                            <Button
                                                ...
                                                disabled={!transferDest || transferMontant <= 0}
                                            />
                                        </Card>
                                    </div>
                        ...
                        <TabPanel header="Historique Transferts" leftIcon="pi pi-history mr-2">
                            ...
                            >
                                <Column field="dateVirement" header="Date" body={(r: VirementInterne) => formatDate(r.dateVirement)} sortable style={{ width: '10%' }} />
                                <Column field="reference" header="Reference" style={{ width: '20%' }} />
                                <Column field="codeCaisseSource" header="Source" style={{ width: '12%' }} />
                                <Column field="codeCaisseDest" header="Destination" style={{ width: '12%' }} />
                                <Column field="montant" header="Montant (FBu)" body={(r: VirementInterne) => formatNumber(r.montant)} style={{ width: '13%', textAlign: 'right' }} sortable />
                                <Column field="libelle" header="Libelle" style={{ width: '18%' }} />
                                <Column
                                    field="status"
                                    header="Statut"
                                    body={(r: VirementInterne) => (
                                        <Tag
                                            value={r.status === 'PENDING_RECEIPT' ? 'EN ATTENTE' : r.status}
                                            severity={r.status === 'COMPLETED' ? 'success' : r.status === 'PENDING_RECEIPT' ? 'warning' : 'info'}
                                            icon={r.status === 'PENDING_RECEIPT' ? 'pi pi-clock' : undefined}
                                        />
                                    )}
                                    style={{ width: '8%' }}
                                />
                                <Column field="executedAt" header="Execute le" body={(r: VirementInterne) => formatDateTime(r.executedAt)} style={{ width: '12%' }} />
                            </DataTable>
                        </TabPanel>
...
export default function Page() {
    return (
        <ProtectedPage requiredAuthorities={['ACCOUNTING_CASH_MANAGEMENT', 'GUICHET_CAISSE', 'CAISSE_VALIDATE_CLOSING']}>
            <GestionCaissePage />
        </ProtectedPage>
    );
}
