...
const RapportDemandesSituationPage = () => {
    ...
    const [reportData, setReportData] = useState<StatementReport[]>([]);
    ...
    useEffect(() => {
        if (reportApi.data) {
            ...
            const filtered = dataArray.filter((i: any) => i.requestType === 'SITUATION');
    ...
    const generateReport = () => {
        ...
        params.append('requestType', 'SITUATION');
    ...
    const exportToExcel = () => {
        if (reportData.length === 0) { toast.current?.show({ severity: 'warn', summary: 'Attention', detail: 'Aucune donn\u00e9e' }); return; }
        let csv = '\uFEFF' + ['N\u00b0 Demande', 'Date', 'Client', 'N\u00b0 Client', 'Agence', 'Frais', 'Statut', 'Livr\u00e9 \u00e0', 'Cr\u00e9\u00e9 par'].join(';') + '\n';
        reportData.forEach(r => { csv += [r.requestNumber, r.requestDate, r.clientName, r.clientNumber, r.branchName, r.feeAmount, statusLabels[r.status] || r.status, r.deliveredToName || '', r.userAction || ''].map(c => `"${c || ''}"`).join(';') + '\n'; });
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `rapport_demandes_situation_${new Date().toISOString().split('T')[0]}.csv`; link.click();
    };

    const statusTemplate = (row: StatementReport) => {
        ...
        <div className="card">
            ...
            <div className="flex align-items-center justify-content-between mb-4">
                <div className="flex align-items-center gap-2">
                    ...
                    <div><h2 className="m-0">Rapport des Demandes de Situation</h2><p className="m-0 text-500">Suivi des demandes de situation de compte</p></div>
                </div>
            ...
            {loading ? <div className="flex justify-content-center p-5"><ProgressSpinner /></div> : (
                <DataTable value={reportData} paginator rows={10} rowsPerPageOptions={[10, 25, 50]} emptyMessage="Aucune donn\u00e9e. Veuillez g\u00e9n\u00e9rer un rapport." className="p-datatable-sm" stripedRows>
                    ...
                    <Column field="deliveredToName" header="Livr\u00e9 \u00e0" sortable />
...
export default function ProtectedPageWrapper() {
    return <ProtectedPage requiredAuthorities={['EPARGNE_REPORT']}><RapportDemandesSituationPage /></ProtectedPage>;
}
