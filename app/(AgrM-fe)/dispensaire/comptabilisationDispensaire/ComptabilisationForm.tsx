...
import { ComptabilisationRequest, TransferParams } from './Comptabilisation';

interface ComptabilisationFormProps {
    onGenererEntree: (values: ComptabilisationRequest) => void;
    onGenererSortie: (values: ComptabilisationRequest) => void;
    onGenererConsommation: (values: ComptabilisationRequest) => void;
...
const ComptabilisationForm: React.FC<ComptabilisationFormProps> = ({ 
    onGenererEntree, 
    onGenererSortie,
    onGenererConsommation,
    onTransfer,
    loading,
    transferLoading 
}) => {
    const [formData, setFormData] = useState<ComptabilisationRequest>({
        magasin: '',
        dateDebut: new Date(),
        dateFin: new Date(),
        numeroPiece: '',
    ...
    const [operationType, setOperationType] = useState<'entree' | 'sortie' | 'consommation' | ''>('');

    const handleInputChange = (field: keyof ComptabilisationRequest, value: any) => {
    ...
    const handleSubmit = (type: 'entree' | 'sortie' | 'consommation') => {
        ...
        if (type === 'entree') {
            ...
            onGenererConsommation(formData);
        }
    };
    ...
    const magasins = [
        { label: 'Magasin Principal', value: 'MAG001' },
    ...
    return (
        <Card title="Comptabilisation du Dispensaire" className="p-fluid">
            <div className="formgrid grid">
                ...
                <div className="field col-12 md:col-6 flex align-items-end gap-2">
                    ...
                    />
                    <Button
                        label="Générer Consommation"
                        icon="pi pi-shopping-cart"
                        onClick={() => handleSubmit('consommation')}
                        loading={loading && operationType === 'consommation'}
