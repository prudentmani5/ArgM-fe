...
interface StkSortieFormProps {
    ...
    loadEntreeData: (numeroPiece: string) => Promise<void>;
}
...
const handleNumeroPieceBlur = async (e: React.FocusEvent<HTMLInputElement>) => {
    try {
        const numeroPiece = e.target.value.trim(); // Nettoyez la valeur
        if (!numeroPiece) return; // Si vide, ne rien faire

        console.log("Vérification du numéro de pièce:", numeroPiece); // Debug
        
        // 1. Vérifier si le numéro existe
        const exists = await checkNumeroPiece(numeroPiece);
        setNumeroPieceExists(!exists);
        
        console.log("Numéro existe:", exists); // Debug

        // 2. Si existe ET loadEntreeData est disponible
        if (!exists && loadEntreeData) {
            console.log("Chargement des données d'entrée..."); // Debug
            await loadEntreeData(numeroPiece);
            console.log("Données chargées avec succès"); // Debug
        }
    } catch (error) {
        console.error("Erreur dans handleNumeroPieceBlur:", error);
        toast.current?.show({
            severity: 'error',
            summary: 'Erreur',
            detail: 'Problème lors de la vérification du numéro',
            life: 3000
        });
    }
};
        ...
        <div className="card p-fluid">
            ...
            <div className="formgrid grid">
                <div className="field col-12 md:col-4">
                    ...
                    />
                    {numeroPieceExists && <small className="p-error">Ce numéro existe déjà. Les données correspondantes seront chargées.</small>}
                </div>
