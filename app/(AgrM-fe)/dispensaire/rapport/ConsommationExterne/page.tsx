...
const RapportConsommationPage: React.FC = () => {
  ...
  const fetchRapport = async (params: RapportConsommationParams) => {
    ...
    try {
      ...
      // Grouper par ayant droit au lieu de matricule
      const grouped = response.data.reduce((acc: RapportConsommationGrouped[], item: RapportConsommation) => {
        const existingGroup = acc.find(group => group.ayantDroitId === item.ayantDroit);
        if (existingGroup) {
          ...
          acc.push({
            ayantDroitId: item.ayantDroit,
            nomAyantDroit: item.nomAyantDroit || 'Non spécifié',
    ...
    <div className="flex justify-content-between align-items-center mb-3">
      ...
      <span className="p-input-icon-left" style={{ width: '40%' }}>
        ...
        <InputText
          ...
          placeholder="Rechercher par ayant droit ou montant..."
          className="w-full"
        />
      </span>
      <Button
        ...
        onClick={() => {
          if (expandedRows) {
            ...
            groupedData.forEach(data => {
              expanded[data.ayantDroitId] = true;
            });
      ...
      >
        <div className="card">
          ...
          >
            ...
            <Column field="ayantDroitId" header="ID Ayant Droit" sortable />
            <Column field="nomAyantDroit" header="Nom Ayant Droit" sortable />
            <Column
