...
const SaisieIndemniteComponent = () => {
    ...
    const loadSaisieIndemniteToEdit = (data: any) => {
        if (data) {
            const editData = Object.assign(new SaisieIndemnite(), data);
            // Map API fields (prenom/nom) to form fields (employeeFirstName/employeeLastName)
            editData.employeeFirstName = data.prenom || data.employeeFirstName || '';
            editData.employeeLastName = data.nom || data.employeeLastName || '';

            setEditSaisieIndemniteDialog(true);
            setSaisieIndemniteEdit(editData);
            ...
            // Set selected periode paie for edit mode - search in both open periods and year periods
            const periodePaie = periodePaies.find(p => p.periodeId === data.periodeId)
                || periodePaiesByYear.find(p => p.periodeId === data.periodeId);
            if (periodePaie) setSelectedPeriodePaie(periodePaie);
        }
    };

    const closeSaisieIndemnite = (data: any) => {
        if (confirm('Êtes-vous sûr de vouloir clôturer cette indemnité?')) {
            fetchData(null, 'Put', `${baseUrl}/api/grh/ficheEmploye/saisie-indemnites/close/${data.indemniteId}/${data.periodeId}`, 'closeSaisieIndemnite');
        }
    };
        ...
        <>
            ...
            >
                <SaisieIndemniteForm
                    ...
                    periodePaies={[...periodePaies, ...periodePaiesByYear.filter(p => !periodePaies.some(op => op.periodeId === p.periodeId))]}
