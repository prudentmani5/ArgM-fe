...
const SaisieRetenueComponent = () => {
    ...
    useEffect(() => {
        ...
        // Handle delete retenue response - reload data with current filter
        if (data && callType === 'deleteSaisieRetenue') {
            accept('success', 'Succès', 'La retenue a été supprimée avec succès.');
            if (filterPeriodeId) {
        ...
        // Handle errors for delete/activate operations
        if (error && callType === 'deleteSaisieRetenue') {
            const errorMessage = (error as any)?.message || 'Impossible de supprimer la retenue.';
    ...
    const deleteSaisieRetenue = (data: any) => {
        // Find the period to check if it's closed
        const periode = periodePaiesByYear.find(p => p.periodeId === data.periodeId);

        // Check if period is closed (dateCloture is not empty)
        if (periode && periode.dateCloture) {
            accept('error', 'Suppression impossible', 'La période de paie est clôturée. Impossible de supprimer cette retenue.');
            return;
        }

        if (confirm('Êtes-vous sûr de vouloir supprimer cette retenue? Cette action est irréversible.')) {
            fetchData(null, 'Delete', `${baseUrl}/api/grh/paie/saisie-retenues/delete/${data.id}`, 'deleteSaisieRetenue');
        }
    };
    ...
    const optionButtons = (data: any): React.ReactNode => {
        ...
        // Check if the period is open (can delete)
        const periode = periodePaiesByYear.find(p => p.periodeId === data.periodeId);
        ...
        return (
            <div className='flex flex-wrap gap-2'>
                ...
                {isPeriodOpen && (
                    <Button
                        icon="pi pi-trash"
                        onClick={() => deleteSaisieRetenue(data)}
                        raised
                        severity='danger'
                        tooltip="Supprimer"
                        size="small"
    ...
    // Calculate total montant from filtered data
    const calculateTotal = () => {
        return filteredSaisieRetenues.reduce((sum, item) => sum + (item.montant || 0), 0);
    };
            ...
            <TabView onTabChange={tableChangeHandle} activeIndex={activeIndex}>
                ...
                <TabPanel header="Tous">
                    <div className='grid'>
                        ...
                        {filteredSaisieRetenues.length > 0 && (
                            <div className='col-12'>
                                <div className='card bg-primary'>
                                    <div className='flex justify-content-between align-items-center'>
                                        <h3 className='m-0 text-white'>Total Montant</h3>
                                        <h2 className='m-0 text-white'>{formatCurrency(calculateTotal())}</h2>
                                    </div>
                                </div>
                            </div>
                        )}
