...
const PrintableListingRetenueBhb = React.forwardRef<HTMLDivElement, PrintableListingRetenueBhbProps>(
    ({ data }, ref) => {
        ...
        // Keep-together strategy inspired by VirementBancairePdfService:
        // Split employees so the last few rows + total + signature stay on the same page
        const KEEP_TOGETHER_COUNT = 3;
        const employees = data.employees;
        const shouldSplit = employees.length > KEEP_TOGETHER_COUNT;
        const mainEmployees = shouldSplit ? employees.slice(0, employees.length - KEEP_TOGETHER_COUNT) : [];
        const tailEmployees = shouldSplit ? employees.slice(employees.length - KEEP_TOGETHER_COUNT) : employees;
        ...
        const renderEmployeeRow = (emp: typeof employees[0], index: number) => (
            <tr key={index}>
        ...
        const mainContent = (
            <div style={{ padding: '14px' }}>
                ...
                {/* Main Employee Table (all rows except last few) */}
                {shouldSplit && (
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                        {tableHeader}
                        <tbody>
                            {mainEmployees.map(renderEmployeeRow)}
                        </tbody>
                    </table>
                )}

                {/* Keep-together block: last few rows + total + signature stay on same page */}
                <div style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                        {tableHeader}
                        <tbody>
                            {tailEmployees.map(renderEmployeeRow)}
                            {totalRow}
                        </tbody>
                    </table>
                    {signatureSection}
