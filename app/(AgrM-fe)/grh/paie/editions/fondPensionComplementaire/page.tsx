...
import PrintableFondPensionComplementaire from './PrintableFondPensionComplementaire';
...
const FondPensionComplementaireComponent = () => {
    ...
    const handleLoadFpc = () => {
        ...
        setLoading(true);
        const url = `${baseUrl}/api/grh/paie/fond-pension-complementaire/${selectedPeriode.periodeId}`;
        fetchFpc(null, 'Get', url, 'loadFpc');
    };

    const handlePrint = useReactToPrint({
        contentRef: printableRef,
        documentTitle: `FPC_BHB_${selectedPeriode?.periodeId || 'report'}`,
        pageStyle: `
            @page {
                size: A4 portrait;
    ...
    return (
        <>
                ...
                <div className="formgrid grid mb-4">
                    ...
                    <div className="field col-12 md:col-4 flex align-items-end gap-2">
                        <Button
                            ...
                            loading={loading}
                        />
                        {fpcData && bankGroup && bankGroup.employees.length > 0 && (
                            <Button
                                icon="pi pi-print"
                                label="Imprimer"
                                severity="info"
            ...
            {/* Print Dialog */}
            <Dialog
                header="Apercu d'impression - Fond de Pension Complementaire"
            ...
            >
                <div style={{ overflowX: 'auto' }}>
                    {bankGroup && fpcData && (
                        <PrintableFondPensionComplementaire
                            ref={printableRef}
                            bankGroup={bankGroup}
                            periodeLibelle={fpcData.periodeLibelle}
                        />
                    )}
                </div>
            </Dialog>
