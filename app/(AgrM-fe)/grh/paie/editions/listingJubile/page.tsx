...
import {
    ListingJubileResponseDto,
    ListingJubileEmployeeDto,
    formatCurrency,
    getFullName
} from './ListingJubile';
import { API_BASE_URL } from '@/utils/apiConfig';
import PrintableListingJubile from './PrintableListingJubile';

const ListingJubileComponent = () => {
    ...
    const [reportData, setReportData] = useState<ListingJubileResponseDto | null>(null);
    ...
    useEffect(() => {
        if (reportDataResponse && callType === 'loadReport') {
            setReportData(reportDataResponse as ListingJubileResponseDto);
            setLoading(false);
    ...
    const handleLoadReport = () => {
        ...
        setLoading(true);
        const url = `${baseUrl}/api/grh/paie/listing-jubile/${selectedPeriode.periodeId}`;
        fetchReport(null, 'Get', url, 'loadReport');
    };

    const handlePrint = useReactToPrint({
        contentRef: printableRef,
        documentTitle: `ListingJubile_${selectedPeriode?.periodeId || 'report'}`,
        pageStyle: `
            @page {
                size: A4 portrait;
                margin: 10mm;
    ...
    return (
        <>
            ...
            <div className="card">
                <h5>Listing Assurance Jubile</h5>
                    ...
                    <div>
                            ...
                            <div className="grid">
                                <div className="col-12 md:col-6">
                                    <div className="text-lg font-bold text-primary mb-2">
                                        ASSURANCE JUBILE : {reportData.periodeLibelle}
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <span className="font-bold">{reportData.compte}</span>
                                    </div>
                                </div>
                        ...
                        >
                            ...
                            <Column
                                ...
                                body={(row: ListingJubileEmployeeDto) => (
                                    <span className="text-red-700">{getFullName(row)}</span>
                                )}
                            />
                            <Column
                                header="Base"
                                body={(row: ListingJubileEmployeeDto) => formatCurrency(row.base)}
                            ...
                            <Column
                                header="Montant"
                                body={(row: ListingJubileEmployeeDto) => formatCurrency(row.montant)}
                                style={{ textAlign: 'right', width: '120px' }}
                            />
                        </DataTable>
            ...
            {/* Print Dialog */}
            <Dialog
                header={`Apercu d'impression - Assurance Jubile`}
...
export default ListingJubileComponent;
