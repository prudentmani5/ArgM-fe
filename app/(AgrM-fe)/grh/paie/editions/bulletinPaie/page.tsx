...
const BulletinPaieComponent = () => {
    ...
    // SaisiePrime and SaisieIndemnite data for bulletin
    ...
    const { data: primesData, fetchData: fetchPrimes, callType: primesCallType } = useConsumApi('');
    const { data: indemnitesData, fetchData: fetchIndemnites, callType: indemnitesCallType } = useConsumApi('');
    const { data: primeParametresData, fetchData: fetchPrimeParametres, callType: primeParametresCallType } = useConsumApi('');
    const { data: indemniteParametresData, fetchData: fetchIndemniteParametres, callType: indemniteParametresCallType } = useConsumApi('');
    ...
    // Handle prime parameters data
    useEffect(() => {
        if (primeParametresData && primeParametresCallType === 'loadPrimeParametres') {
            setPrimeParametres(Array.isArray(primeParametresData) ? primeParametresData : [primeParametresData]);
        }
    }, [primeParametresData, primeParametresCallType]);

    // Handle indemnite parameters data
    useEffect(() => {
        if (indemniteParametresData && indemniteParametresCallType === 'loadIndemniteParametres') {
            setIndemniteParametres(Array.isArray(indemniteParametresData) ? indemniteParametresData : [indemniteParametresData]);
        }
    }, [indemniteParametresData, indemniteParametresCallType]);

    // Handle primes data for bulletin
    useEffect(() => {
        if (primesData && primesCallType === 'loadBulletinPrimes') {
            const primes = Array.isArray(primesData) ? primesData : [primesData];
            const primeInstances = primes.map((p: any) => Object.assign(new SaisiePrime(), p));
            setSaisiePrimes(primeInstances);
        }
    }, [primesData, primesCallType]);

    // Handle indemnites data for bulletin
    useEffect(() => {
        if (indemnitesData && indemnitesCallType === 'loadBulletinIndemnites') {
            const indemnites = Array.isArray(indemnitesData) ? indemnitesData : [indemnitesData];
            const indemniteInstances = indemnites.map((i: any) => Object.assign(new SaisieIndemnite(), i));
            setSaisieIndemnites(indemniteInstances);
        }
    }, [indemnitesData, indemnitesCallType]);
    ...
    useEffect(() => {
        if (paieData && paieCallType === 'loadBulletin') {
            ...
            // Also fetch the employee's retenues, primes, and indemnites for this period
            fetchRetenues(
                null,
                'Get',
            ...
            fetchPrimes(
                null,
                'Get',
                `${baseUrl}/api/grh/paie/saisie-primes/employee/${matriculeId}/period/${selectedPeriodeId}`,
                'loadBulletinPrimes'
            );
            fetchIndemnites(
                null,
                'Get',
                `${baseUrl}/api/grh/ficheEmploye/saisie-indemnites/employee/${matriculeId}/period/${selectedPeriodeId}`,
                'loadBulletinIndemnites'
            );
        }
                                        ...
                                        <table className="w-full">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td className="py-1 text-right">{formatAmount(saisiePaie.montantPreste)}</td>
                                                </tr>
                                                <tr>
                                                    <td className="py-1 text-left">Jours Prestés:</td>
                                                    <td className="py-1 text-right">{saisiePaie.preste.toFixed(2)}</td>
                                                </tr>
                                                <tr>
                                                ...
                                                {saisiePrimes.map((prime, index) => (
                                                    <tr key={`prime-${prime.id || index}`}>
                                                        <td className="py-1 text-left">{getPrimeLabel(prime.codePrime)}:</td>
                                                        <td className="py-1 text-right">{formatAmount(prime.montant)}</td>
                                                    </tr>
                                                ))}
                                                {/* Display SaisieIndemnite items */}
                                                {saisieIndemnites.map((indemnite, index) => (
                                                    <tr key={`ind-${indemnite.indemniteId || index}`}>
                                                        <td className="py-1 text-left">{getIndemniteLabel(indemnite.codeInd)}:</td>
                                                        <td className="py-1 text-right">{formatAmount(indemnite.montant)}</td>
                                                    </tr>
                                                ))}
                    ...
                    {saisiePaie && (
                        <div style={{ display: 'none' }}>
                            <PrintableBulletinPaie
                                ...
                                saisiePrimes={saisiePrimes}
                                saisieIndemnites={saisieIndemnites}
