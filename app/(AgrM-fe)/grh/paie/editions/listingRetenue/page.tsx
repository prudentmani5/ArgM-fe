...
import {
    ListingRetenueResponseDto,
    ListingRetenueEmployeeDto,
    RetenueParametre,
    formatCurrency,
    getFullName
} from './ListingRetenue';
import { API_BASE_URL } from '@/utils/apiConfig';
import PrintableListingRetenue from './PrintableListingRetenue';

const ListingRetenueComponent = () => {
    ...
    const [selectedRetenue, setSelectedRetenue] = useState<RetenueParametre | null>(null);
    const [periodes, setPeriodes] = useState<PeriodePaie[]>([]);
    const [retenues, setRetenues] = useState<RetenueParametre[]>([]);
    const [reportData, setReportData] = useState<ListingRetenueResponseDto | null>(null);
    const [loading, setLoading] = useState<boolean>(false);
    const [loadingPeriodes, setLoadingPeriodes] = useState<boolean>(false);
    const [loadingRetenues, setLoadingRetenues] = useState<boolean>(false);

    // API hooks
    const { data: periodesData, fetchData: fetchPeriodes } = useConsumApi('');
    const { data: retenuesData, fetchData: fetchRetenues } = useConsumApi('');
    const { data: reportDataResponse, error: reportError, fetchData: fetchReport, callType } = useConsumApi('');

    // Load retenues on mount
    useEffect(() => {
        setLoadingRetenues(true);
        fetchRetenues(null, 'Get', `${baseUrl}/api/grh/paie/retenues/findall`, 'loadRetenues');
    }, []);

    // Handle retenues response
    useEffect(() => {
        if (retenuesData) {
            const data = Array.isArray(retenuesData) ? retenuesData : [retenuesData];
            setRetenues(data);
            setLoadingRetenues(false);
        }
    }, [retenuesData]);
    ...
    useEffect(() => {
        if (reportDataResponse && callType === 'loadReport') {
            setReportData(reportDataResponse as ListingRetenueResponseDto);
            setLoading(false);
    ...
    const handleLoadReport = () => {
        ...
        if (!selectedRetenue) {
            showToast('warn', 'Validation', 'Veuillez selectionner une retenue.');
            return;
        }

        setLoading(true);
        const url = `${baseUrl}/api/grh/paie/listing-retenue/${selectedPeriode.periodeId}/${selectedRetenue.codeRet}`;
        fetchReport(null, 'Get', url, 'loadReport');
    };

    const handlePrint = useReactToPrint({
        contentRef: printableRef,
        documentTitle: `ListingRetenue_${selectedRetenue?.codeRet || ''}_${selectedPeriode?.periodeId || 'report'}`,
        pageStyle: `
            @page {
                size: A4 portrait;
    ...
    const retenueOptionTemplate = (option: RetenueParametre) => (
        <span>{option.codeRet} - {option.libelleRet}</span>
    );

    const selectedRetenueTemplate = (option: RetenueParametre | null) => {
        if (option) {
            return <span>{option.codeRet} - {option.libelleRet}</span>;
        }
        return <span>Selectionner une retenue</span>;
    };
        ...
        <>
            ...
            <div className="card">
                ...
                <div className="formgrid grid mb-4">
                    ...
                    <div className="field col-12 md:col-3">
                        <label htmlFor="retenue">Retenue</label>
                        <Dropdown
                            id="retenue"
                            value={selectedRetenue}
                            options={retenues}
                            onChange={(e) => setSelectedRetenue(e.value)}
                            optionLabel="libelleRet"
                            itemTemplate={retenueOptionTemplate}
                            valueTemplate={selectedRetenueTemplate}
                            placeholder="Selectionner"
                            className="w-full"
                            loading={loadingRetenues}
                            filter
                            showClear
                        />
                    </div>
                    ...
                    <div>
                            ...
                            <div className="grid">
                                <div className="col-12 md:col-6">
                                    ...
                                    <div className="mb-1">
                                        ...
                                        <span className="text-blue-700 font-bold">{reportData.retenueCode} - {reportData.retenueLibelle}</span>
                                    </div>
                                    {reportData.banqueLibelle && (
                                        <div className="mb-1">
                                    ...
                                    {reportData.compte && (
                                        <div>
                                            ...
                                            <span className="text-blue-700 font-bold">{reportData.compte}</span>
                                        </div>
                                    )}
                                </div>
                        ...
                        >
                            ...
                            <Column
                                ...
                                body={(row: ListingRetenueEmployeeDto) => (
                            ...
                            <Column
                                header="Montant"
                                body={(row: ListingRetenueEmployeeDto) => formatCurrency(row.montant)}
                ...
                {!loading && reportData && reportData.employees.length === 0 && (
                    <div className="text-center p-5 text-500">
                        ...
                        <p>Aucune donnee trouvee pour cette periode et cette retenue.</p>
                    </div>
                )}
            </div>
            ...
            {/* Print Dialog */}
            <Dialog
                header={`Apercu d'impression - Listing Retenue`}
...
export default ListingRetenueComponent;
