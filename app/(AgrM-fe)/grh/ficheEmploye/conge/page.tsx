...
const CongeComponent = () => {
    ...
    const processedSearchCongeData = useRef<any>(null);
    ...
    useEffect(() => {
        ...
        if (searchCongeData && searchCongeCallType === 'searchConges' && searchCongeData !== processedSearchCongeData.current) {
            processedSearchCongeData.current = searchCongeData;
            const searchResults = Array.isArray(searchCongeData) ? searchCongeData : [searchCongeData];
            setFilteredConges(searchResults);
        }

        if (searchCongeError && searchCongeCallType === 'searchConges' && processedSearchCongeData.current !== 'error') {
            processedSearchCongeData.current = 'error';
        ...
        // Handle delete conge
        if (data && data !== processedData.current && callType === 'deleteConge') {
            processedData.current = data;
            accept('info', 'OK', 'Le congé a été supprimé avec succès.');
            processedSearchCongeData.current = null;
            loadCongesByExercice(searchExercice);
        }

        // Handle errors for create/update/delete
        if (error && (callType === 'createConge' || callType === 'updateConge' || callType === 'deleteConge') && processedData.current !== 'error') {
            ...
            if (callType === 'createConge') {
                ...
                accept('warn', 'A votre attention', 'La suppression n\'a pas été effectuée.');
            }
            setBtnLoading(false);
        }
    ...
    const handleSearchConges = (matriculeValue: string, exerciceValue: number) => {
        ...
        processedSearchCongeData.current = null; // Reset to allow processing new search
    ...
    const handleAfterApiCall = (chosenTab: number) => {
        if (error !== null && chosenTab === 0) {
            ...
            if (callType === 'createConge') {
                ...
                processedSearchData.current = null; // Reset to allow new searches
                processedCalcInfo.current = null; // Reset to allow new calculation info
                processedSearchCongeData.current = null; // Reset to allow reload
    ...
    const confirmDeleteConge = (congeData: Conge) => {
        confirmDialog({
            message: `Êtes-vous sûr de vouloir supprimer ce congé de ${congeData.nom || ''} ${congeData.prenom || ''} ?`,
            header: 'Confirmation de suppression',
            icon: 'pi pi-exclamation-triangle',
            acceptLabel: 'Oui',
            rejectLabel: 'Non',
            acceptClassName: 'p-button-danger',
            accept: () => handleDeleteConge(congeData.congeId!),
        });
    };

    const handleDeleteConge = (congeId: number) => {
        processedData.current = null;
        fetchData(null, 'Delete', `${baseUrl}/api/grh/conges/delete/${congeId}`, 'deleteConge');
    };

    const optionButtons = (data: any, options: any): React.ReactNode => {
        return (
            <div className='flex flex-wrap gap-2'>
                <Button icon="pi pi-pencil" onClick={() => loadCongeToEdit(data)} raised severity='warning' tooltip="Modifier" />
                {hasAuthority('RH_MANAGER') && (
                    <Button icon="pi pi-trash" onClick={() => confirmDeleteConge(data)} raised severity='danger' tooltip="Supprimer" />
                )}
            </div>
        );
    };
