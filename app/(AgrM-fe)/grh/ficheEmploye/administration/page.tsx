...
const EmployeeDetailsComponent = () => {
    ...
    const handleSubmitEdit = () => {
        ...
        fetchData(employeeDetailEdit, 'Put', baseUrl + '/api/employee-details/' + employeeDetailEdit.matriculeId, 'updateEmployeeDetail');
    };
    ...
    const filteredEmployeeDetails = employeeDetails.filter((emp: any) => {
        if (!globalFilterValue.trim()) return true;
        const search = globalFilterValue.trim().toLowerCase();
        return (
            emp.matriculeId === globalFilterValue.trim() ||
    ...
    const loadEmployeeDetailToEdit = (data: any) => {
        if (data) {
            setEditEmployeeDetailDialog(true);
            setEmployeeDetailEdit(data);
            setEmployeeNom(data.nom || '');
            setEmployeePrenom(data.prenom || '');
        }
    };
    ...
    const renderSearch = () => {
        return (
            <div className="flex justify-content-between">
                <Button type="button" icon="pi pi-filter-slash" label="Réinitialiser" outlined onClick={clearFilterEmployeeDetail} />
                <span className="p-input-icon-left">
                    <i className="pi pi-search" />
                    <InputText
                        value={globalFilterValue}
                        onChange={(e) => setGlobalFilterValue(e.target.value)}
                        placeholder="Recherche par matricule, nom, prénom ou email"
                    />
                </span>
            </div>
        );
