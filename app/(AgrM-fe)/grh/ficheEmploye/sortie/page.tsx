...
const SortieComponent = () => {
    ...
    // Main API hook: create / update / loadSorties
    useEffect(() => {
        if (data && callType === 'loadSorties') {
    ...
    // Sortie list search hook
    useEffect(() => {
        if (searchSortieData && searchSortieCallType === 'searchSorties') {
            const paged = searchSortieData as any;
            const content = Array.isArray(paged.content) ? paged.content : [];
            setFilteredSorties(content);
            setTotalRecords(paged.totalElements ?? 0);
        }
    ...
    }, [searchSortieData, searchSortieError]);
    ...
    const handleSearchSorties = (matriculeValue: string, exerciceValue: number, page: number = 0, rows: number = 10) => {
        if (matriculeValue.trim() === '') {
            loadSortiesByExercice(exerciceValue, page, rows);
            return;
        }
        fetchSearchSortie(null, 'Get',
            `${baseUrl}/api/grh/sorties/matricule/${encodeURIComponent(matriculeValue.trim())}/exercice/${exerciceValue}?page=${page}&size=${rows}`,
            'searchSorties'
        );
    };
    ...
    const handleSearchInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSearchTerm(value);
        setLazyState({ first: 0, rows: lazyState.rows, page: 0 });

        const timeoutId = setTimeout(() => {
            handleSearchSorties(value, searchExercice, 0, lazyState.rows);
        }, 300);
    ...
    const handleSearchExerciceChange = (e: any) => {
        ...
        setLazyState({ first: 0, rows: lazyState.rows, page: 0 });
        handleSearchSorties(searchTerm, value, 0, lazyState.rows);
    };

    const handleSubmit = () => {
        const dateSortieS = formatLocalDateTime(dateSortie);
    ...
    const handleAfterApiCall = (chosenTab: number) => {
        if (error !== null && chosenTab === 0) {
            ...
            if (callType === 'createSortie') {
                ...
                loadSortiesByExercice(searchExercice, lazyState.page, lazyState.rows);
            }
        }
        setBtnLoading(false);
    };
    ...
    const clearFilterSortie = () => {
        ...
        setLazyState({ first: 0, rows: lazyState.rows, page: 0 });
        loadSortiesByExercice(currentYear, 0, lazyState.rows);
    };

    const onPage = (event: any) => {
        const newLazy = { first: event.first, rows: event.rows, page: event.page };
        setLazyState(newLazy);
        handleSearchSorties(searchTerm, searchExercice, event.page, event.rows);
    };
    ...
    const loadSortiesByExercice = (exercice: number, page: number = 0, rows: number = 10) => {
        fetchSearchSortie(null, 'Get', `${baseUrl}/api/grh/sorties/exercice/${exercice}?page=${page}&size=${rows}`, 'searchSorties');
    };

    const tableChangeHandle = (e: TabViewTabChangeEvent) => {
        if (e.index === 1) {
            setLazyState({ first: 0, rows: lazyState.rows, page: 0 });
            loadSortiesByExercice(searchExercice, 0, lazyState.rows);
        } else {
            setSortie(new Sortie());
            setEmployeeName('');
        }
