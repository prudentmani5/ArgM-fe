...
const RapportCaissierPage: React.FC = () => {
  ...
  const fetchRapport = async (values: { dateDebut: Date; dateFin: Date;  importateurId: number }) => {
    setLoading(true);
    ...
    try {
      const response = await fetch(
        `${baseUrl}/entryPayements/rapportClient?debut=${debut}&fin=${fin}&caissier=${values.importateurId}`
      );

      if (!response.ok) throw new Error('Erreur réseau');

      const data = await response.json();
      const caissierNom = data[0]?.nomClient || 'Inconnu';

      setSearchParams({ ...values, caissierNom });
        ...
        <PDFDownloadLink
          document={
            <RapportCaissierPdf
              ...
              caissierNom={searchParams.caissierNom}
            />
          }
          fileName={`rapport-caissier-${searchParams.caissierNom}-${format(searchParams.dateDebut, 'yyyyMMdd')
