...
const RapportCaissierPage: React.FC = () => {
  ...
  const fetchRapport = async (values: { dateDebut: Date; dateFin: Date; caissierId: number }) => {
    setLoading(true);
    ...
    try {
      const response = await fetch(
        `${baseUrl}/entryPayements/rapportCaissier?debut=${debut}&fin=${fin}&caissier=${values.caissierId}`
      );

      if (!response.ok) throw new Error('Erreur réseau');

      const data = await response.json();
      const caissierNom = data[0]?.nomCaissier || 'Inconnu';
