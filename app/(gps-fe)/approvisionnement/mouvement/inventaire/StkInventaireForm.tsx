...
interface StkInventaireFormProps {
    ...
    addDetail: () => void;
    removeDetail: (index: number, inventaireDetailsId?: number) => void;
...
const StkInventaireForm: React.FC<StkInventaireFormProps> = ({
    ...
    const confirmAndDeleteDetail = (rowIndex: number, inventaireDetailsId?: number) => {
        const article = getArticleById(details[rowIndex]?.articleId);
        const articleName = article?.libelle || "cet article";
        
        confirmDialog({
            message: `Êtes-vous sûr de vouloir supprimer ${articleName} ?`,
            header: 'Confirmation de suppression',
            icon: 'pi pi-exclamation-triangle',
            acceptLabel: 'Oui',
            rejectLabel: 'Non',
            accept: () => executeDelete(rowIndex, inventaireDetailsId),
            acceptClassName: 'p-button-danger'
        });
    };

    // Fonction pour exécuter la suppression
    const executeDelete = async (rowIndex: number, inventaireDetailsId?: number) => {
        if (inventaireDetailsId !== undefined) {
            // C'est un détail existant qui doit être supprimé de la base de données
            try {
                setDeletingId(inventaireDetailsId);
                await axios.delete(`${BASE_URLD}/delete/${inventaireDetailsId}`);
                
                // Supprimer de l'état local
                removeDetail(rowIndex, inventaireDetailsId);
                
                toast.current?.show({
                    severity: 'success',
                    summary: 'Succès',
                    detail: 'Article supprimé avec succès de la base de données',
                    life: 3000
                });
            } catch (error) {
                ...
                toast.current?.show({
                    severity: 'error',
                    summary: 'Erreur',
                    detail: 'Échec de la suppression de l\'article de la base de données',
                    life: 3000
                });
            } finally {
            ...
            // C'est un nouveau détail, on le supprime simplement du state
            removeDetail(rowIndex);
            toast.current?.show({
                severity: 'info',
                summary: 'Information',
                detail: 'Ligne supprimée du formulaire',
                life: 3000
            });
        }
    };

    // Fonction pour afficher le nom de l'article avec catalogue
    const renderArticleDisplay = (rowData: StkInventaireDetails) => {
            ...
            <div className="mt-4">
                ...
                >
                            ...
                            return (
                                <div>
                                    <Dropdown
                                        ...
                                        showClear
                                    />
                                    {/* Afficher le catalogue uniquement sous le nom de l'article */}
                                    {rowData.articleId && renderArticleDisplay(rowData)}
                                </div>
                            );
                        }}
                    ...
                    <Column
                        header="Actions"
                        body={(rowData, { rowIndex }) => {
                            const isDeleting = deletingId === rowData.inventaireDetailsId;
                            const isLoading = isDeleting || (loading && rowData.inventaireDetailsId !== undefined);
                            
                            return (
                                <Button
                                    icon="pi pi-trash"
                                    className="p-button-rounded p-button-danger p-button-text"
                                    onClick={() => confirmAndDeleteDetail(rowIndex, rowData.inventaireDetailsId)}
                                    tooltip="Supprimer"
                                    loading={isLoading}
