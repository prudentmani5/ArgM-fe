...
import { ComptabilisationGrouped, ComptabilisationRequest, TransferParams } from './Comptabilisation';
import Comptabilisation from './Comptabilisation';
...
export default function ComptabilisationPage() {
    ...
    const [searchParams, setSearchParams] = useState<ComptabilisationRequest | null>(null);
    ...
    const genererEntree = async (values: ComptabilisationRequest) => {
        setLoading(true);
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/comptabilisation/generer-entree`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ...values,
                        dateDebut: format(startOfDay(values.dateDebut), 'yyyy-MM-dd'),
                        dateFin: format(endOfDay(values.dateFin), 'yyyy-MM-dd')
                    })
                }
            );
    ...
    const genererSortie = async (values: ComptabilisationRequest) => {
    setLoading(true);
    try {
        const requestBody = {
            ...values,
        ...
        console.log('Sending sortie request:', requestBody);

        const response = await fetch(
            `${API_BASE_URL}/api/comptabilisation/generer-sortie`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        // Vérifier si la réponse est une erreur 400 avec le message spécifique
        if (response.status === 400) {
            const errorText = await response.text();
            
            // Vérifier si c'est l'erreur "array vide"
            if (errorText.includes('SQL array must not be empty') || 
                errorText.includes('array must not be empty')) {
                showToast('info', 'Information', 'Aucune sortie à comptabiliser pour les critères sélectionnés');
                return; // Arrêter l'exécution ici
            }
            
            // Pour les autres erreurs 400
            throw new Error(`Erreur lors de la génération des sorties: ${errorText}`);
        }
        ...
        // Vérifier si c'est une erreur de tableau vide (au cas où le format du message change)
        if (error instanceof Error && 
            (error.message.includes('SQL array must not be empty') || 
             error.message.includes('array must not be empty'))) {
            showToast('info', 'Information', 'Aucune sortie à comptabiliser pour les critères sélectionnés');
        } else {
    ...
    const genererConsommation = async (values: ComptabilisationRequest) => {
        setLoading(true);
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/comptabilisation/generer-consommation`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
            ...
            if (!response.ok) throw new Error('Erreur lors de la génération des consommations');

            await fetchBrouillard();
            setSearchParams(values);
            showToast('success', 'Succès', 'Consommations comptabilisées avec succès');
            setVisibleDialog(true);
        } catch (error) {
            console.error("Erreur:", error);
            showToast('error', 'Erreur', 'Échec de la comptabilisation des consommations');
        } finally {
            setLoading(false);
        }
    };
    ...
    const transferer = async (values: TransferParams) => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/comptabilisation/transferer`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ...
                        dateDebut: format(startOfDay(values.dateDebut), 'yyyy-MM-dd'),
                        dateFin: format(endOfDay(values.dateFin), 'yyyy-MM-dd'),
                        dateTransfert: format(startOfDay(values.dateTransfert), 'yyyy-MM-dd')
                    })
                }
            );
            ...
            <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
                <TabPanel header="Comptabilisation">
                    <ComptabilisationForm
                        ...
                        onGenererConsommation={genererConsommation}
                ...
                <div className="card">
                    ...
                    >
                        ...
                        <Column 
                            field="totalDebit" 
                            header="Total Débit" 
                            body={(data: ComptabilisationGrouped) => formatCurrency(data.totalDebit)}
                            sortable 
                        />
                        <Column 
                            field="totalCredit" 
                            header="Total Crédit" 
                            body={(data: ComptabilisationGrouped) => formatCurrency(data.totalCredit)}
                            sortable 
                        />
                    </DataTable>
