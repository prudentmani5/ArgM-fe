...
export const ManutentionPDF = ({ invoice }) => {
    ...
    // Calcul des montants pour le détail du magasinage/gardiennage
    const montantMagasin = finalTonnageArrondi * (invoice.montPrixMagasin || 0) * (invoice.duree || 0);
    const montantMagasin37 = finalTonnageArrondi * (invoice.montPrixMagasin37 || 0) * (invoice.duree37 || 0);
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.section}>
                    <View style={{ textAlign: 'center' }}>
                        <Text>Facture Proforma de {invoice.typeFacture} No {invoice.sortieId} , le {new Date().toLocaleDateString('fr-FR')}</Text>
                    ...
                    <View style={styles.tableContainer}>
                        ...
                        {/* VERSION COMPACTE ALIGNÉE AVEC JAVA */}
                        <View style={styles.centeredHeader}>
                            <Text style={styles.centeredHeaderText}>
                                {invoice.typeConditionId === 'VE' ? 'GARDIENAGE' : 'MAGASINAGE'}
                                : {formatCurrency(invoice.montGardienage || invoice.montMagasinage)}
                            </Text>
                        </View>

                        <View style={styles.centeredMagasinDetail}>
                            <Text style={styles.periodeText}>
                                {/* Condition pour afficher la période selon le type de facture */}
                                {invoice.typeFacture === 'Menutention' ? (
                                    <Text style={styles.periodeText}>
                                        Du {formatDate(dateDebut)} au {formatDate(invoice.dateValidation)}
                                    </Text>
                                    ...
                                    <Text style={styles.periodeText}>
                                        Du {formatDate(invoice.dateValidation)} au {formatDate(invoice.dateDerniere)}
                                    </Text>
                                )}
                            </Text>
                            ...
                            {invoice.typeConditionId === 'VE' ? (
                                // AFFICHAGE VÉHICULE (GARDINNAGE)
                                <>
                                    <Text style={styles.calculLine}>
                                        {formatNumber(finalTonnageArrondi)} × {formatNumber(invoice.montPrixMagasin)} × {invoice.duree || 0} Jours  = {formatCurrency(montantMagasin)}
                                    </Text>
                                    {invoice.duree37 > 0 && (
                                        <Text style={styles.calculLine}>
                                            {formatNumber(finalTonnageArrondi)} × {formatNumber(invoice.montPrixMagasin37)} × {invoice.duree37} Jours  = {formatCurrency(montantMagasin37)}
                                        </Text>
                                    )}
                                </>
                            ) : (
                                // AFFICHAGE MARCHANDISE STANDARD (MAGASINAGE)
                                <>
                                    <Text style={styles.calculLine}>
                                        {formatNumber(finalTonnageArrondi)} × {formatNumber(invoice.montPrixMagasin)} × {invoice.duree || 0} Jours =  {formatCurrency(montantMagasin)}
                                    </Text>
                           ...
                           {/* <View style={styles.separateur}></View>
                            ...
                            <Text style={[styles.calculLine, { fontWeight: 'bold', color: '#1e40af' }]}>
                                Total: {formatCurrency(invoice.typeConditionId === 'VE' ? invoice.montGardienage : invoice.montMagasinage)}
                            </Text>
                           */}
                        </View>
