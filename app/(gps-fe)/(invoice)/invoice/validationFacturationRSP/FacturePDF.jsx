...
export const FacturePDF = ({ invoice = {} }) => {
    ...
    const renderCalculationDetails = (item) => {
        switch (item) {
            ...
            case 'surtaxeColisLourd':
                return `${formatCurrencyNumber(poidsKg)} kg × ${formatCurrency(invoice.surtaxeClt)}`;
            case 'montSalissage':
    ...
    const calculateTotalManutention = () => {
        const manutentions = [
            Number(invoice.manutBateau) || 0,
            Number(invoice.manutCamion) || 0,
            Number(invoice.surtaxeColisLourd) || 0,
            Number(invoice.montSalissage) || 0,
            Number(invoice.montArrimage) || 0,
            Number(invoice.peage) || 0,
            Number(invoice.montRedev) || 0,
            Number(invoice.montPalette) || 0,
            Number(invoice.montPesMag) || 0,
            Number(invoice.montLais) || 0,
            Number(invoice.montEtiquette) || 0,
            Number(invoice.montFixationPlaque) || 0
        ];
            ...
            <Page size="A4" style={styles.page}>
                    ...
                    <View style={[styles.section, styles.column]}>
                        ...
                        <View style={styles.row}>
                            ...
                            <Text style={styles.value}>
                                {invoice.typeFacture === 'Supplement'
                                    ? `${formatDate(invoice.dateSortie)} - ${formatDate(invoice.dateSupplement)}`
                                    : invoice.typeFacture === 'Solde'
                                        ? `${formatDate(invoice.dateSortie)} - ${formatDate(invoice.dateDerniereSortie)}`
                ...
                <View style={styles.tableContainer}>
                    ...
                    <View style={styles.tableRow}>
                        ...
                        <Text style={[styles.calculationCell, { width: '30%' }]}>
                            {(() => {
                                const montant37 = isGardiennage ? invoice.montGardienage37 : invoice.montMagasinage37;
                                if (montant37 > 0 && invoice.duree37 > 0 && invoice.montMag37 > 0) {
                                    return `${formatCurrencyNumber(tonnageArrondi)} × ${formatCurrency(invoice.montMag37)} × ${invoice.duree37}j`;
                                } else if (montant37 > 0 && tonnageArrondi > 0) {
                                    // Calculate duree37 from the amount if not available
                                    const calculatedDuree = invoice.montMag37 > 0 ? Math.round(montant37 / (tonnageArrondi * invoice.montMag37)) : 0;
                                    const prixUnit = calculatedDuree > 0 ? Math.round(montant37 / (tonnageArrondi * calculatedDuree)) : 0;
                                    return calculatedDuree > 0
                                        ? `${formatCurrencyNumber(tonnageArrondi)} × ${formatCurrency(prixUnit)} × ${calculatedDuree}j`
                                        : `Montant période étendue`;
                                }
                                return 'Période étendue';
                            })()}
                        </Text>
                        <Text style={[
                            styles.tableCell,
                ...
                {invoice.factureSignature && (
                    <View style={{ marginTop: 10, textAlign: 'center', fontSize: 7 }}>
                        <Text style={{ fontWeight: 'bold' }}>Signature Électronique OBR:</Text>
                        <Text style={{ marginTop: 3 }}>{invoice.factureSignature}</Text>
                    </View>
                )}
            </Page>
        </Document>
